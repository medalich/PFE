...
type RestOf<T extends IActivite | NewActivite> = Omit<T, 'dateDebut' | 'dateFin'> & {
...
export class ActiviteService {
  ...
  protected convertDateFromClient<T extends IActivite | NewActivite | PartialUpdateActivite>(activite: T): RestOf<T> {
    return {
      ...activite,
      dateDebut: activite.dateDebut?.format(DATE_FORMAT) ?? null,
      dateFin: activite.dateFin?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restActivite: RestActivite): IActivite {
    return {
      ...restActivite,
      dateDebut: restActivite.dateDebut ? dayjs(restActivite.dateDebut) : undefined,
      dateFin: restActivite.dateFin ? dayjs(restActivite.dateFin) : undefined,
    };
  }

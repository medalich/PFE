...
describe('Client e2e test', () => {
  ...
  const clientSample = {
    refClient: 'deploy interactive b',
    nom: 'Licensed Île-de-France fault-tolerant',
    prenom: 'bluetooth plug-and-play',
  };
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="refClient"]`).type('quantifying cultivat').should('have.value', 'quantifying cultivat');

      cy.get(`[data-cy="nom"]`).type('XSS').should('have.value', 'XSS');

      cy.get(`[data-cy="prenom"]`).type('calculating').should('have.value', 'calculating');

      cy.get(`[data-cy="contact"]`).type('Licensed Buckinghamshire Steel').should('have.value', 'Licensed Buckinghamshire Steel');
